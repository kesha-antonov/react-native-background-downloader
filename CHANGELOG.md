...
- **Android 14+: two concurrent downloads could share one notification:** notification IDs were derived as `NOTIFICATION_ID_BASE + hash(configId) % 100000`, so downloads whose config IDs hashed to the same slot posted to the same ID - their progress updates overwrote each other, and whichever ended first took the notification down for the other, permanently on the UIDT path where the job's end policy removes it. Across a batch of 150 downloads that is about a one-in-ten chance. IDs are now assigned per live download, still starting the search at the hash so a download keeps the ID it has always had unless another live download holds it. The completion notification's ID comes from the same assignment, so it inherits the uniqueness.
- **Android: a download the service could never start hung forever with no event ([#174](https://github.com/kesha-antonov/react-native-background-downloader/pull/174)):** when a download fell back to the foreground service and the system refused to start it - a background start on Android 12+, say - the start was parked waiting for a service connection that never came. No `begin`, no `error`, and the JS task sat pending for the rest of the app's life. A download that hasn't reached the service within 15 seconds now fails with an error the app can handle, and the parked start is dropped if the connection lands afterwards.
- **Android 14+: a download whose job hadn't started yet was invisible, and could end up tracked twice ([#173](https://github.com/kesha-antonov/react-native-background-downloader/pull/173)):** `getExistingDownloadTasks()` listed UIDT downloads from an in-memory registry that is only filled in `onStartJob`, so a job still waiting on its network constraint - or, right after a process restart, every job the app had scheduled - was missing from the result even though its transfer was still coming. The same in-memory check gated force-stop recovery at startup, where it is always empty, so a scheduled download was also recovered as a *paused* one: JS then showed it as paused while the job started downloading on its own. Both now ask the JobScheduler, which holds the job whether it is running or waiting, and a scheduled download is reported with the same status `DownloadManager` uses for a queued one. Recovery also stopped clearing the whole snapshot store on startup, since a scheduled download that hasn't transferred a byte has no progress callback to re-save its snapshot and would be lost if the app were force-stopped again.
- **Android 14+: concurrent downloads could silently kill each other through colliding JobScheduler IDs ([#172](https://github.com/kesha-antonov/react-native-background-downloader/pull/172)):** each download's job ID was derived as `JOB_ID_BASE + hash(configId) % 10000`, and `JobScheduler.schedule()` on an ID that already exists *replaces* the job holding it. Two downloads hashing to the same slot therefore left one of them scheduled and the other gone - no error, no event, just a download that never progressed and a `stop()`/`pause()` that acted on the wrong job. This is not a corner case: across the 150 concurrent downloads the app-wide job quota allows, the birthday bound puts a collision at roughly two chances in three, and a single collision only needs two downloads whose IDs happen to hash together. A download's job is now found by the config ID the job already carries in its extras, which makes the JobScheduler itself the registry - correct across process death by construction, with nothing to migrate for downloads scheduled by an older version - and a download that needs a new job takes the first ID no live job holds, the host app's own jobs in that range included.
- **Android 14+: starting a large batch of downloads could crash the app on the JobScheduler quota ([#171](https://github.com/kesha-antonov/react-native-background-downloader/pull/171)):** every download on Android 14+ is scheduled as its own user-initiated data transfer job, but `JobScheduler` holds at most 150 distinct jobs per app and throws `IllegalStateException` ("Apps may not schedule more than 150 distinct jobs") from `schedule()` once that quota is full. The call was unguarded, so an app queueing a few hundred downloads - or one queueing fewer while WorkManager and other libraries drew from the same app-wide pool - died on an uncaught exception instead of downloading. Scheduling now checks the app's pending-job count first and keeps headroom for the rest of the app, and the `schedule()` call itself is guarded, so a full quota degrades to the existing foreground-service path (the same fallback already used when UIDT scheduling fails) instead of terminating the process. Re-scheduling a download that is already pending still works at the limit, since it replaces its job rather than adding one.

### ✨ Features

- **Android 14/15: fresh downloads now run as user-initiated data transfer jobs:** only Android 16+ used them for a new download; on Android 14 and 15 a download started through `DownloadManager` and only became a UIDT job after a pause/resume. That split meant the library's notification features - grouping, the completion notification, the Cancel action, per-download titles - did nothing for a download until it had been paused once, and downloads missed out on UIDT's exemption from App Standby quotas. A download now takes a UIDT job first on any Android 14+ device. `DownloadManager` stays as the fallback for when a job can't be scheduled, most importantly a `download()` call from the background, where the system refuses user-initiated jobs - so background-started downloads keep working exactly as before.
- **`maxParallelDownloads` now applies on Android ([#174](https://github.com/kesha-antonov/react-native-background-downloader/pull/174)):** the config was documented as iOS-only and was a no-op on Android, where the library's own downloader started a thread and a socket for every download at once - a batch of a few hundred meant a few hundred threads. It now caps the transfers that downloader runs at once (default 4, matching iOS and the JS default), with the rest waiting for a slot and the next one starting as soon as a running transfer completes, fails, is paused or is stopped. A download that pauses or is cancelled while waiting never starts. The cap covers the mechanism used on Android 16+ and every `DownloadManager`/UIDT fallback; downloads that run through `DownloadManager` or the JobScheduler are queued by those schedulers instead, so they keep transferring in the background exactly as before. On iOS the config is unchanged (the session's maximum connections per host).
...
- **Android unit tests:** the Android side had no test infrastructure at all, so the scheduling work above was verified by reading and by hand. There is now a Robolectric + MockWebServer harness and 38 tests over the parts that are easy to get subtly wrong: job-ID assignment (the collision cases use two config IDs that genuinely share a hash code), the JobScheduler quota guard and the headroom it leaves the rest of the app, re-scheduling a download that already has a job, reporting and pausing a job that hasn't started, notification-ID assignment, and the transfer concurrency cap - where a MockWebServer dispatcher holds responses open so a running download really does occupy its slot, and the tests cover pausing, cancelling and resuming a download while it waits for one. Run them with `yarn test:android`; a GitHub Actions workflow runs the JS and Android suites on push and pull request.
