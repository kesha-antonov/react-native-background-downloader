...
describe('16KB Memory Page Size Support', () => {
  test('should have Android build configuration for 16KB page alignment', () => {
    ...
    // MMKV 1.3.14+ (LTS) supports both armeabi-v7a and 16KB page sizes
    // MMKV 2.0.0+ also supports 16KB page sizes but dropped armeabi-v7a
    expect(buildGradleContent).toMatch(/mmkv-shared:[0-9]+\.[0-9]+\.[0-9]+/)
  })
  ...
  test('should have example app configured for 16KB support', () => {
    ...
    // Check if example app is Expo-based (no android folder until prebuild)
    const exampleAppJsonPath = path.join(__dirname, '../example/app.json')
    if (fs.existsSync(exampleAppJsonPath)) {
      // Expo-based example app - check app.json configuration instead
      const appJson = JSON.parse(fs.readFileSync(exampleAppJsonPath, 'utf8'))

      // Should have Expo configuration
      expect(appJson.expo).toBeDefined()

      // Should have the library plugin configured
      expect(appJson.expo.plugins).toContainEqual('@kesha-antonov/react-native-background-downloader')

      // Note: With Expo, android/ios folders are generated by prebuild
      // and SDK versions are managed by Expo SDK version
      return
    }

    // Legacy React Native CLI example app
  ...
  test('MMKV dependency version should support 16KB page sizes', () => {
    ...
    if (mmkvMatch) {
      ...
      // MMKV 1.3.14+ supports 16KB page sizes AND armeabi-v7a (32-bit)
      // MMKV 2.0.0+ also supports 16KB page sizes but dropped armeabi-v7a
      // We use 1.3.x LTS for maximum device coverage
      const supports16kb =
        major >= 2 ||
        (major === 1 && minor > 3) ||
        (major === 1 && minor === 3 && patch >= 14)
      expect(supports16kb).toBe(true)
    }
  })
})
